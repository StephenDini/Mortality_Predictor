...
app.config["MONGO_URI"] = "mongodb://localhost:27017/mort_app"
mongo = PyMongo(app)

# define the route for the HTML page
@app.route('/')
def index():
    mort = mongo.db.mort.find_one()
    return render_template('index.html', mort=mort)
...
def scrape():
    ...
    data_by_country = scraping.scrape_all()
    # Now that we've gathered new data, we need to update the database using .update_one()
    mort.update_one({}, {'$set':data_by_country}, upsert=True)
